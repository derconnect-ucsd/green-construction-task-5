"""
Apply figure/export edits to harmonics-study notebooks.
Run this after clearing all outputs (Jupyter: Cell -> All Output -> Clear) and saving,
so the notebook JSON is valid. Or run on a copy of the notebook.

Edits applied:
- Add TEST_ID (9B / 9C) and use in all .png filenames
- Raw data plot: 6 rows, remove loadbank subplot, height 1800, font
- PMU verification: vertical_spacing=0.25, font, update_annotations
- All figures: font=dict(size=14), title_font=dict(size=18), update_annotations(font_size=16)
- Remove loadbank from raw_data; IEEE annotation font 10 -> 14
- frequency_from_waveform: font in layout
"""
import json
import re
import sys
...
BASE = _os.path.join(_script_dir, "..", "analysis")
...
def main():
    for nb_name, test_id in [("harmonics-study_test_9B_moxion.ipynb", "9B"), ("harmonics-study_test_9C.ipynb", "9C")]:
        ...
        try:
            ...
            print(nb_name, ": JSON invalid (clear all outputs and save, then re-run). Error:", e)
            continue
